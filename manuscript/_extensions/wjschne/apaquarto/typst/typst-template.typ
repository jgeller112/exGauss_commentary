...
#let doc(..args) = apa-layout(
  ...
  justify: true,
  // Upstream default is "none" (no running head at all) with the page number
  // moved to the foot instead. This project wants the running head kept even
  // in doc mode, matching the older apaquarto template's behavior, so the
  // page number goes back to the header (via headerstyle "running") and
  // pagenumbering is left at apa-layout's default of `none` rather than
  // duplicating the number at the foot too.
  headerstyle: "running",
  updatepagecounter: true,
  ..args,
)
