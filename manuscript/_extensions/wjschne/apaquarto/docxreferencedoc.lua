...
--- header1.xml (even pages) and header3.xml (first page) in this reference
--- document carry no running-head control at all -- just a blank paragraph
--- in the Header style -- because the section turns on neither "different
--- first page" nor "different odd and even" (no w:titlePg, no
--- w:evenAndOddHeaders), so Word never displays them. Some other renderers
--- (a docx viewer, an online converter) are less strict and show whichever
--- header a type attribute names regardless, which is when these blank
--- pages surface with no running head at all. Since patch_header only
--- touches the header holding the real control, give this one a plain text
--- run instead, in the same paragraph the reference document already has.
---
--- The marker key must not contain a hyphen or any other Lua pattern magic
--- character: marker_pattern() splices it into a pattern unescaped, and an
--- unescaped "-" is a lazy quantifier there, not a literal dash, which
--- silently breaks get_marker/strip_marker matching (they never match, so
--- the old run never gets cleared and keeps piling up on every render).
local function patch_plain_header(header, runninghead)
  if not runninghead then return nil end
  if find_running_head(header) then return nil end

  local stripped = strip_marker(header, "runningheadplain")
  local _, ppr_end = stripped:find("<w:pPr>.-</w:pPr>")
  if not ppr_end then return nil end
  local p_end = stripped:find("</w:p>", ppr_end + 1, true)
  if not p_end then return nil end

  --- Whatever sits between the paragraph properties and the paragraph's end
  --- the first time this runs -- empty in the reference document -- so a
  --- later render replaces its own last run rather than piling up another.
  local original = get_marker(header, "runningheadplain") or
    stripped:sub(ppr_end + 1, p_end - 1)

  local run = "<w:r><w:t xml:space=\"preserve\">" ..
    escape_xml(runninghead) .. "</w:t></w:r>"
  local marker = make_marker("runningheadplain", original)
  return stripped:sub(1, ppr_end) .. marker .. run .. stripped:sub(p_end)
end
...
function Pandoc(doc)
  ...
  for _, entry in ipairs(archive.entries) do
    ...
    elseif entry.path:match(header_pattern) then
      --- The header holding the running-head control gets it filled in;
      --- the other headers (even/first) have no such control, so they get
      --- the running head as a plain run instead.
      xml = entry:contents()
      patched = patch_header(xml, runninghead) or patch_plain_header(xml, runninghead)
    end
